...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cleaning_type = $conn->real_escape_string($_POST['cleaning_type']);
    ...
    $payment_number = isset($_POST['payment_number']) ? $conn->real_escape_string($_POST['payment_number']) : null; // Menggunakan null jika payment_number tidak diisi
    $date = $conn->real_escape_string($_POST['date']);
    $time = $conn->real_escape_string($_POST['time']);

    // Gunakan prepared statement untuk menghindari SQL injection
    $stmt = $conn->prepare("INSERT INTO orders (cleaning_type, duration, name, email, phone, address, pay_method, total_cost, pay_num, date, time, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sisssssssssi", $cleaning_type, $duration, $name, $email, $phone, $address, $payment_method, $total_cost, $payment_number, $date, $time, $_SESSION['user_id']);
    
    // Eksekusi statement
    if ($stmt->execute()) {
        // Redirect sebelum output apa pun
